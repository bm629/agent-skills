...
**Breaking from v5.0.0 (v6.0.0 — skill intent + resolution fields):** every `manifest.skills` entry now carries a required per-project `purpose` + a non-empty `requirements` list (populated for every skill, feeding the downstream dashboard approval gate's display + strict match), and every skill AND role entry carries `resolved_id` + `match_status` (`complete`|`partial`|`none`|null; both `null` at discovery, written by the gate). The validator adds a resolution-consistency check (complete/partial ⇒ `resolved_id` set; none/null ⇒ null); emitted `manifest.version` → 3.

**Breaking from v4.0.0 (v5.0.0 — manifest review roles):** each document entry's singular `role` becomes `roles: [author, reviewer]` (exactly two, fixed order) and carries both its authoring and reviewing skill; `manifest.roles` derives the archetype-level author/reviewer pair (`document-author`/`document-reviewer`, `designer`/`design-reviewer`) as pure personas (`skills`/`tools` dropped); emitted `manifest.version` → 2. The validator adds a roles referential-integrity + archetype→pair consistency check.
