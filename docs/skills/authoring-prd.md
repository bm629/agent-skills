...
4. **Apply the per-section method** — problem *evidenced* (not asserted); 2–4 measurable success metrics each with a target + measurement method, defined before building; explicit MVP vs out-of-scope/non-goals + release criteria; functional + non-functional requirements with targets; user stories with checkable acceptance criteria; risks + a pre-mortem; jargon-free, no vague adjectives standing in for requirements.
5. **Self-check against the plannability bar** before handing off (below).

## The plannability quality bar

A PRD is "good" when the build can be planned from it. The eight conditions: (1) evidenced problem; (2) defined users; (3) measurable success metrics; (4) defensible MVP boundary with non-goals; (5) concrete features + acceptance criteria specific enough to derive milestones; (6) risks + open questions surfaced; (7) clear, unambiguous, no vague adjectives; (8) grounded, not assumed (thin-idea assumptions flagged; too-thin → blocker). A companion PRD-review skill asserts the same bar, so author and reviewer stay aligned.
