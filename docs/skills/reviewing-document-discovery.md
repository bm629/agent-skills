...
(13) **all five manifest meta-sections populated** — `capabilities:` contains `docs` (always) and `design` (if `ui.has_ui: true`); no build-level capability; `roles:`, `skills:`, `tools:`, `amendments:` all present and non-empty (except `amendments: []` which is an empty array by design at discovery time); `roles:` holds the full author/reviewer pair for every archetype present and every role entry is a pure persona (no `skills`/`tools`); all skill entries have `version: null` and `source: null`; (14) **`capability` scalar on every document entry** — each document entry has `capability: "docs"` or `capability: "design"` as a string scalar, not `capabilities: [...]` (array form from v2.x).
...
- **Single-sourced bar** — the same fourteen conditions `project-document-discovery`'s Step-9 Self-check (v5.0.0) uses, asserted independently (nine content/DAG + three output-contract + two meta-section).
