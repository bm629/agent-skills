...
**44 numbered conditions**, contiguous, grouped per kind — plus one group headed `## Every kind`,
...
is this document's declaration of the result rather than a second copy of it. It lands FOUR
...
reviews of the planted set, which grounded a never-queried expansion, two fabricated source facts
and a transferability limit that never reached the index, and could file none of them — a reviewer
that can ground a finding and cannot land it is telling you the bar has the hole.
