...
- **Single-sourced bar** — shared with the dedicated `reviewing-architecture-doc` reviewer (the 11-condition gate), so produce and review don't drift.
- **capability-record-aware** — when the full capability list is injected by the caller, component decomposition and `depends_on` DAG are derived from it; graceful fallback when absent.
