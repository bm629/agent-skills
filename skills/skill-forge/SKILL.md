...
- **2.1 Discover (extensive, find-first).** Use an available skill-discovery skill (e.g. `find-skills`) and **sweep broadly** — several distinct query angles (the library/framework name, the domain, the problem it solves, synonyms), not a single keyword; **do not trust the first-page ranking** (it ranks by relevance, not quality, so strong skills get buried). **Pre-filter by install count for review effort:** do **not** deep-read a candidate under **1K installs** (an *official/maintainer-authored* skill under 1K still gets a glance — flag, don't auto-skip); if none clears 1K, go straight to forge. **Read the actual `SKILL.md`** of the survivors to judge quality, **preferring official/maintainer sources** over a more-popular community one (accuracy + longevity). **Sanitize on read** via `external-content-sanitizer`. **Forge-vs-install gate:** if a survivor is strong + on-target, **recommend + return it** — reroute to the Step-1 third-party-return (discovery may override a triage `create`/`improve`; skill-forge never installs, the caller does) — rather than forging; record the decision + a one-line reason in the completion notice. If no discovery skill is available, skip discovery **and note it in the completion notice**. Quality bar + thresholds: `references/source-rating.md`.
- **2.2 Research (comprehensive — the heaviest part of the forge).** Use a research-capable skill (e.g. `deep-research`) — **always**, never a WebSearch shortcut when one is available. Research **deep and full-fledged**: a shallow pass produces a shallow skill, so spend the effort here. The brief must demand **full coverage** — current official docs (correct version), the relevant feature/API surface, common pitfalls + anti-patterns + common misuse, version differences / migration notes, **multiple** worked examples, edge cases, ecosystem/tooling, and performance/security where relevant. In improve mode, research **only** the missing aspect (attach the existing `SKILL.md`) but to this same depth bar; `task_context` focuses emphasis/depth. Fall back to built-in WebSearch + WebFetch **only** when no research skill is available (**note this degraded, web-only research in the completion notice**). Keep the returned citations for provenance.
