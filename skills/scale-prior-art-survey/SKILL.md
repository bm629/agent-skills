...
25. **Write one area per ADR unit**, each with its `evidence[]` of EPISODE IDS, its `confidence`
    ...
    among the backing episodes' sources, across every evidence site rather than `evidence[]`
    alone, written with the date in the form the source carries it and read as a caveat on the
    area rather than folded into `confidence`. Null ONLY where every backing source is undated,
    and a null on a dated corpus is refused. **All FOUR evidence sites carry EPISODE IDS** — the
