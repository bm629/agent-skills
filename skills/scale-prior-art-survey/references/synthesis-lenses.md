...
| 8 | Currency | EVERY distinct `published_date` among the sources of the episodes backing the area — every evidence site, not just `evidence[]` — written into `currency`, EACH in the form its source carries it, and read as a caveat on the whole area rather than folded into `confidence`. Not "the oldest": `published_date` is free text — a publication date, a benchmark result date, a documentation version or an incident date — so there is no ordering to lean on, and naming them all gives the reader the span rather than one end of it. Null only where every backing source is undated. This lens had no output field for eight review cycles, which is why its formula names one now. |
