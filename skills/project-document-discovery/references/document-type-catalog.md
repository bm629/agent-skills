...
Broad-but-shallow reference for selecting a project's document set. **Seven lifecycle bands** (everyone walks them, proportionally) **plus four domain overlays** (added only when the project's domain triggers them). Each entry is **name — what it is — when needed — what it feeds — `depends_on`** (the documents that *inform* it, copied into the manifest and pruned to the project's set). The list is **illustrative and open-ended**: if a project needs a type not here, research it and add it. Depth on *how to write* any of these belongs to a per-document authoring skill, not here.

## Band 0 — Project management / planning

- **Project charter / SOW** — the mandate: scope, stakeholders, objectives, deliverables; for any funded/team effort; *precedes and feeds* the PRD (it has no upstream dependency). `depends_on: []`
- **Roadmap / release plan** — work sequenced into milestones/releases; for managed multi-milestone efforts; *depends_on* the PRD/feature specs (you schedule from defined requirements — it sits dependency-downstream of Band 2, not before it). `depends_on: [prd, feature-specs]`
- *(proportional — managed/regulated efforts only, not core)* **RAID log** — running Risks, Assumptions, Issues, Dependencies; for actively-managed projects (`raid-log depends_on: [charter-sow, prd]`). **Risk register / risk-management plan** — catalogued risks + mitigations; for enterprise/regulated efforts (`risk-register depends_on: [charter-sow, prd]`).

## Band 1 — Analysis / discovery

- **Problem statement** — the need being solved; when there's any ambiguity about the "why"; feeds the PRD. `depends_on: []`
- **Market / competitor scan (MRD)** — the landscape and differentiation; for commercial products; feeds the PRD/business case. `depends_on: [problem-statement]`
- **Business case (BRD)** — business goals, constraints, success criteria; for funded/enterprise efforts; feeds the PRD. `depends_on: [problem-statement, market-scan]`
- **User research / personas** — who the users are and their behaviors; for user-facing products; feeds the PRD + design. `depends_on: [problem-statement]`
- **Success metrics** — how success is measured; whenever outcomes must be tracked; feeds release/analytics. `depends_on: [problem-statement, business-case]`

## Band 2 — Product / requirements (usually load-bearing)

- **PRD** — what to build: features, behaviors, interactions; for almost any feature-bearing product; feeds design, architecture, planning. **The keystone document.** `depends_on: [charter-sow, problem-statement, market-scan, business-case, user-research, success-metrics]`
- **Feature list / feature specs** — the enumerated features and their detail; whenever there is more than a trivial feature set; feeds decomposition/planning. `depends_on: [prd]`
- **User stories + acceptance criteria** — features from the user's view with done-conditions; agile teams; feeds tests + decomposition. `depends_on: [prd, feature-specs]`
- **SRS** (software requirements spec) — a detailed functional blueprint; for regulated/large systems; feeds architecture + tests. `depends_on: [prd, feature-specs]`

## Band 3 — Design / UX (for UI-bearing products)

- **User flows** — start-to-finish journeys through tasks; for any non-trivial UI; feeds wireframes + tests. `depends_on: [prd, feature-specs, user-research]`
- **Information architecture / sitemap** — pages/screens and their links; for multi-screen products; feeds wireframes + navigation. `depends_on: [prd, user-flows]`
- **Wireframes** — low-fidelity screen structure (no visual styling); for any UI; feeds mockups + component specs + front-end build. `depends_on: [prd, user-flows, information-architecture, design-system]`
- **Hi-fi mockups** — visual design of screens; when visual fidelity matters; feeds front-end build. `depends_on: [wireframes, design-system]`
- **Design system / tokens** — reusable UI building blocks + styles; for products with many screens/teams; feeds component specs. `depends_on: [prd, user-research]`
- **Component & interaction specs** — how each element behaves, annotated for developers; for complex UIs; feeds front-end build. `depends_on: [design-system, wireframes]`
- **Accessibility notes** — a11y requirements; for public/inclusive products; feeds build + QA. `depends_on: [user-flows, wireframes, design-system]`
...
- **README** — entry point / orientation; for every project; feeds first use + onboarding. `depends_on: [prd]`
- **End-user guide / manual** — how to use the product; for any user-facing product; feeds adoption + support. `depends_on: [prd, feature-specs, user-flows]`
- **Getting-started / quickstart / install guide** — the first-run path; for CLI, library, API, or app; feeds onboarding. `depends_on: [readme, end-user-guide]`
- **API reference docs** — the published, navigable interface docs users read; for any library/SDK/API; feeds integration. *(The human-read deliverable — distinct from the Band-4 API specification, which is the OpenAPI build-feed.)* `depends_on: [api-spec]`
- **Release notes** — a curated, human summary of what changed per release; for anything shipping iteratively; feeds users + stakeholders. `depends_on: [feature-specs]`
- **Changelog** — a running record of every change by version; for libraries/SDKs and any versioned artifact; feeds maintainers + integrators. `depends_on: []`
...
- **Data lineage / data-flow doc** — where data originates, transforms, and lands; feeds governance + impact analysis. `depends_on: [data-model, architecture-overview]`
- **Eval / experiment plan** — benchmarks, eval harness, success bar; feeds model acceptance. `depends_on: [prd, data-model]`
- **Data contract** — a producer↔consumer schema/SLA commitment; feeds pipeline reliability. `depends_on: [data-model, architecture-overview]`
...
- **Security review / pentest report** — findings from a security assessment; feeds the release gate. `depends_on: [architecture-overview, threat-model]`
- **Compliance mapping** — controls mapped to SOC 2 / ISO 27001 / HIPAA; feeds audit. `depends_on: [architecture-overview, threat-model]`
- **Data processing agreement (DPA)** — data-handling contract terms; feeds legal + privacy. `depends_on: [dpia]`

### Legal / governance overlay
*Trigger: published / open-source, or public commercial.*

- **LICENSE** — the legal license; feeds distribution. `depends_on: []`
- **CONTRIBUTING** — how to contribute; feeds the community. `depends_on: [readme]`
- **CODE_OF_CONDUCT** — community standards; feeds governance. `depends_on: []`
- **SECURITY.md** — how to report a vulnerability; feeds responsible disclosure. `depends_on: []`
- **GOVERNANCE** — roles + decision process; feeds larger OSS projects. `depends_on: []`
- **Terms of Service / Privacy Policy / EULA** — legal terms for users; feeds a public commercial launch. `depends_on: [prd, dpia]`
- **Third-party license / NOTICE attribution** — dependency-license compliance; feeds distribution. `depends_on: [sbom]`

### Regulated / validation overlay
*Trigger: FDA / medical / pharma / safety-critical.*

- **Requirements traceability matrix (RTM)** — requirement → design → test linkage; feeds audit + validation. `depends_on: [srs, feature-specs, test-plan]`
- **Validation protocols (IQ / OQ / PQ)** — install / operational / performance qualification; feeds validation. `depends_on: [srs, rtm]`
- **Validation summary report** — the compiled validation evidence; feeds the regulatory submission. `depends_on: [validation-protocols, rtm]`

**See also (research-on-gap):** documents not catalogued here but sometimes needed — RACI, capacity/scaling plan, backup/restore policy, go-to-market/launch plan, training/enablement plan, configuration-management plan, data-retention policy. Research a type before placing it; never guess its purpose — and give it a `depends_on` (the documents that inform it).
...
## Dependency edges — explicit per entry, pruned to the set

Each catalog entry above carries its explicit **`depends_on`** — *every* document that informs it (not only the strictly-blocking input), so the producer is handed a rich enough upstream set to write a comprehensive document. To assemble a project's graph: take each selected document's `depends_on` from its entry, then **prune each list to the documents actually in this project's set** (drop an edge to a document the project isn't producing). Production then runs in dependency order.

- Edges flow **requirements → design → delivery → docs**; every edge points from a later document to an earlier one. A PRD never depends on the architecture; a charter never depends on the PRD.
- The catalog edge set is **pre-verified acyclic**, and pruning a subset out can never introduce a cycle — so the assembled graph is always acyclic. (The one pair that could cycle, `api-spec` ↔ `data-model`, is deliberately one-directional: `api-spec depends_on: [.. data-model ..]`, while `data-model depends_on: [feature-specs, architecture-overview]` — never the api-spec.)
- **Verify the assembled (pruned) graph is acyclic** as a final safety check.
