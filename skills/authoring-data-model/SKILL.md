...
2. **Relationships & referential integrity** — every relationship states cardinality; an M:N is realized by a junction entity; the FK/reference + owning side named; the **on-delete rule** stated (relational) or **embed-vs-reference + the consistency-without-FK strategy** stated (NoSQL/other).
