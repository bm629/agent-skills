...
| **3 · Conflict** | Pairwise within a dimension group where the constraints do not intersect, plus every disagreement on a NON-COMPARABLE dimension. Its own section, `requires_counsel: true`, **escalated and never resolved**. |
...
A `conflicts[]` row carries `conflict_id`, the `requirement_ids` in tension — at least two, because
a conflict is between obligations — and `why_irreconcilable`.
