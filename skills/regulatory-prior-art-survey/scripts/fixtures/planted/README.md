...
**Each fixture carries ONE defect.** A fixture with two does not test which one a reviewer can
find; it tests which one it happens to look at first.
...
To reproduce a blind run, hand a subagent the reviewing skill, one fixture, AND the clean scope map
— the map is a separate INPUT to a search-output review, not context to be withheld. Two conditions
judge a candidate against the group the map minted, and a reviewer without it cannot exercise
either. Then require a numbered condition in the finding.
