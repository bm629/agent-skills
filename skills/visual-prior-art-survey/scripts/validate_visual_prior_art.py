#!/usr/bin/env python3
"""Deterministic gate for visual prior-art artifacts.

Checks SHAPE and completeness only — schema, enums, ranges, required fields, and arithmetic
that reconciles two records against each other. It never judges whether a convention really
binds, whether a relevance line is persuasive, or whether a corpus was well chosen; those are the
...
#: Most of this corpus is keyed by stable identifiers (ARIA pattern slugs, WCAG criterion
#: numbers, deceptive-pattern slugs) where exclusion terms would be noise. Design SYSTEM names
#: are the exception: Polaris, Primer, Carbon, Fluent, Spectrum and Lightning collide with
#: ordinary language and with unrelated products, and an angle discovering systems BY NAME
#: cannot be made precise afterwards.
COLLISION_PRONE_TYPES = ("design-system",)
...
def record_filename(item_id: str) -> str:
    ...
    a corpus id assembled from an upstream system name may carry characters a filename
    cannot. Downstream stages
...
def validate_keyword_map(doc: dict, registry: dict | None = None) -> list[str]:
    """Validate a UI-pattern vocabulary map.
    ...
    Returns:
        One ``FAIL`` line per violation, empty when clean.
    """
    out = _schema_failures(doc, "ui-pattern-vocabulary-map.schema.json")
    if out:
        return out
        ...
        if g["type"] in COLLISION_PRONE_TYPES and not g.get("negative_terms"):
            out.append(
                _fail(
                    ...
                    "design-system names (Polaris, Primer, Carbon, Fluent) collide with "
                    "ordinary language, so a search built from this group cannot be made "
                    "precise afterwards",
                )
            )
...
def validate_search(doc: dict, mapping: dict, registry: dict | None = None) -> list[str]:
    ...
    map_errs = _schema_failures(mapping, "ui-pattern-vocabulary-map.schema.json")
    if map_errs:
...
_ID_SHAPES = {
    "aria-pattern": re.compile(r"^ARIA-[a-z0-9-]+$"),
    "wcag-criterion": re.compile(r"^WCAG-\d+\.\d+\.\d+$"),
...
def _candidate_failures(doc: dict) -> list[str]:
    ...
    convention is the reviewer's condition, not a shape check.
    """
    out: list[str] = []
    ...
    for cand in doc.get("candidates") or []:
        ...
        if shape and not shape.match(cid):
            out.append(
                _fail(
                    ...
                    "corpus's identifier form; a corpus-scoped id is what makes a convention "
                    "re-findable, and an invented shape is not",
                )
            )

        tf = cand.get("token_format")
        if tf and not tf.strip().upper().startswith("DTCG"):
            out.append(
                _fail(
                    "token-format-dtcg",
                    f"candidate {cid!r} claims token format {tf!r}; the downstream consumer reads "
                    "DTCG, and a claim in another format cannot be handed on unchanged",
                )
            )
        if tf and not any(ch.isdigit() for ch in tf):
            out.append(
                _fail(
                    "token-format-versioned",
                    f"candidate {cid!r} claims a token format with no version; the format is "
        ...
        # `admission.corpus_url` and `corpus_release` are schema-REQUIRED, so their absence is
        # caught before this runs. Nothing semantic is left to check here: whether the corpus is
        # genuinely retrievable and its release genuinely current is the reviewer's condition.
...
#: these hold what a human reads and what synthesis quotes. A design system's component catalog
#: and its DTCG token block live here too (spec W2-Q1), which is why the body is checked at all.
EXTRACT_HEADINGS = ("Statement", "Evidence", "Applicability")
...
def _load_frontmatter(path: Path) -> tuple[dict | None, str | None, list[str]]:
    ...
    reporting it as a rule would tell a caller to edit an artifact that may be fine.

    Args:
        path: The record path.

    Returns:
        ``(frontmatter, body, failures)``; frontmatter and body are ``None`` when unreadable.
    """
    try:
        text = path.read_text()
...
def validate_extract(path: Path | str) -> list[str]:
    ...
    corpus, and whether the authority band is right are the reviewing twin's conditions. This
    gate checks the schema, then the two things a schema cannot express: that a skip carries a
    substantive cause, and that an extracted record's body has the sections synthesis quotes.
    ...
    # gate's — 5a puts it there and #45 forbids diverging. A validator that also enforced it
    # would duplicate the reviewer and make every fixture unnameable.
    ...
    if conv.get("tokens_in_body"):
        if "```dtcg" not in (body or ""):
            out.append(
                _fail(
                    "tokens-block",
                    "tokens_in_body is true but the body carries no ```dtcg block — the register "
                    "row copies that block verbatim, so a missing one silently empties it",
                )
            )
    elif conv.get("id_class") == "design-system" and "```dtcg" in (body or ""):
        out.append(
            _fail(
                "tokens-flag",
                "the body carries a ```dtcg block but tokens_in_body is false — synthesis reads "
                "the flag, so the tokens would never reach the register",
            )
        )
    return out
...
def validate_synthesis(doc: dict, extracts: Path | None = None) -> list[str]:
    ...
    What this gate owns is the part a reviewer cannot check by reading: that every row traces
    to an extract record that exists, and that the coverage arithmetic reconciles.

    ``extracts`` is OPTIONAL because the register is also read on its own, but omitting it
    SKIPS the cross-check rather than passing it — 5e shipped a validator whose evidence check
    silently no-opped without its directory, so the gap was never reported.
    ...
    Returns:
        One ``FAIL <rule>: ...`` line per violation; empty when clean.
    """
    out = _schema_failures(doc, "convention-register.schema.json")
    if out:
        return out
    ...
    for row in rows:
        if row.get("id_class") != "design-system" and "tokens" in row:
            out.append(
                _fail(
                    "tokens-scope",
                    f"{row.get('id')!r}: only a design-system row carries tokens; a token set "
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_map = sub.add_parser("keyword-map", help="validate a UI-pattern vocabulary map")
    ...
    p_syn = sub.add_parser("synthesis", help="validate the convention register")
    ...
    # An extract record is markdown with frontmatter, not a YAML document, so it takes the path
    # rather than a parsed doc — reading it through _read would fail on the body.
    if args.cmd == "extract":
