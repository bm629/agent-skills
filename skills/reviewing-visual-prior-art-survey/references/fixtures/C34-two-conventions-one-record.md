...
convention:
  ...
  governs: collapsible section headers, and separately the modal dialog focus-trap pattern
  applicability:
    applies: true
...
Quoted from the APG pattern page (2026-04), section "WAI-ARIA Roles, States, and Properties":
the button element has `aria-expanded` set to `true` when the controlled region is visible
and `false` when hidden.
