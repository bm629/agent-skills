...
**C9.** `assumptions[]` records what the author had to assume. A map that assumed something and
...
**C10.** Each candidate's `url` RESOLVES to what the row claims it is. The gate checks the field is
present (`admission`); whether it resolves is yours.

**C11.** The admission conjuncts were applied HONESTLY. A `stated_date` copied from the retrieval
...
**C12.** Every unadmitted row's `reason_class` is the one that actually applies, and its `reason`
says what was looked for.

**C13.** A zero is ENUMERATED or BOUNDED, and the cell says which. Look the row up in
...
**C14.** The `ordering` is RE-APPLIABLE by a reader with the same corpus: it names a signal every
...
**C15.** `bound.cap` is the registry's value TRANSCRIBED VERBATIM. Look the angle up in
...
**C16.** `hit: true`'s `dropped_note` names the ordering position reached and the first row that
fell off, and a reader could act on it.

**C17.** **`a1` only** — a candidate's recorded host posture was taken AT FETCH TIME from the host
...
**C18.** **The number is the SOURCE's number.** `measured_value` is verbatim as the source words
it. A converted, rounded or recomputed figure is a finding even when the arithmetic is right.

**C19.** The `evidence_quote` and `claim` pair: the quote SUPPORTS the claim, and the claim does
not reach past it.

**C20.** `evidence_class` fits what the source IS. A vendor's own blog post describing its own
system is not `independent-verification` however measured it is.

**C21.** **`configuration_stated: true` means the configuration is actually stated.** A benchmark
number with no disclosed configuration is not a comparable measurement, however authoritative its
host.

**C22.** **`primary_dimension` is the dimension the episode actually MEASURED** — judged from
...
**C23.** **`transferability` is weighable and INDEPENDENT of `confidence`.** A `reason` that
...
**C24.** **The episode's `confidence` is defensible against the four facts the derivation reads**
...
**C25.** **The `score` is defensible.** The validator checks presence and range
...
**C26.** **The episode's `cause_class` is a FAILURE MODE, not the map's field of the same name.**
...
**C27.** `load_class` sub-keys record what the SOURCE states. A band filled in from the project's
...
**C28.** **For a dimension in `references/load-band-thresholds.md`'s `unsourced_dimensions` list,
...
**C29.** The four body sections `references/extraction-template-guide.md` names say something. The
...
**C30.** A `skipped` record's `detail` is observable, in the forms
...
**C31.** Every `evidence[]` id resolves to an episode that says what the area claims it says. The
gate checks resolution (`synthesis`); whether the episode supports the pattern is yours.

**C32.** The area's `confidence` is the WEAKEST backing class, and the weakest episode is one a
reader would agree is weakest.

**C33.** A `blocks_requirement: true` hard limit really blocks a requirement this project has. It
is the only blocker-producing lens, and a false one costs more than a missed one.

**C34.** `default_pattern` is what the episodes converged on, not what the reviewer would have
chosen.

**C35.** The `migration_trigger` names a condition someone could observe, and its `dimension` is
the axis the trigger is actually on.

**C36.** **An absence is phrased with its receipt.** `open_gap` says what was looked for and where,
not merely that something is unknown.

**C37.** The index's `project_band` EQUALS the map's `meta.classification.scale`, leaf for leaf,
