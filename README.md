...
| [`market-competitive-prior-art-survey`](docs/skills/market-competitive-prior-art-survey.md) | Run the search wave of a systematic MARKET & COMPETITIVE prior-art survey — who already competes, what they offer and charge, how users receive them, and which comparable products died — for a product that does not exist yet. Two procedures over schema-validated contracts: derive a market vocabulary map (five axes — category, capability, job-to-be-done, audience-segment, seed-product — with SKOS-typed expansions, honest provenance, mandatory negative terms on the collision-prone axes, per-angle applicability verdicts and absent-input assumptions), and execute one of 8 angles (alternatives-graph traversal / category-and-capability search / review-corpus mining / app-store listings / package-registry adoption / corporate-status resolution / product-mortality mining / community sentiment) into status-typed coverage cells. The did-not-look versus found-nothing distinction is structural: not-run and vacated outcomes forbid coverage cells outright, only a `reached` cell may carry counts, `kept` must reconcile against the rows that name it, and coverage completeness is checked in BOTH directions. The job-to-be-done axis is the substitute-finder — a set defined by category alone misses non-consumption and 'do it by hand', frequently the true incumbent. Source access is verified per source and part of the contract: G2 is excluded on its Terms of Use (which forbid automated extraction regardless of what robots permits), Trustpilot, Similarweb and Reddit on robots, and the validator rejects a cell or a fallback naming any of them. Every conditional angle trigger must rest on a REQUIRED capability field — a predicate anchored on an optional one fails closed and invisibly, so the angle looks configured and does nothing. Deterministic validator, 42 rules, 94 tests; three planted fixtures pass it and are each wrong, because a green gate says nothing about whether the search was good. v1.1.0. |
| [`reviewing-market-competitive-prior-art-survey`](docs/skills/reviewing-market-competitive-prior-art-survey.md) | Judge a produced market & competitive prior-art artifact — vocabulary map or per-angle search output — against the twenty-seven-condition bar single-sourced with `market-competitive-prior-art-survey` (an acceptance gate, not authoring). Mechanical conditions are discharged by one run of the co-installed producer's validator and deliberately not restated; the judgment conditions are the point — whether the job-to-be-done axis genuinely describes a need or merely paraphrases the category, whether two corroborating angles are actually independent, whether a first-party capability quote states a capability at all or is pure positioning, whether a zero is a zero or a failure wearing one, and whether a rating travels with its denominator. Proportionality is a numbered condition, not a disposition: a thin-but-honest artifact is correct output for a thin market, and revising for thinness invites padding. Every condition names its evidence, and anything ungroundable is an OBSERVATION rather than a finding — an ungrounded finding costs a revise round and, at the cap, parks correct work. Emits `VERDICT: approve\|revise` with condition-named findings, all in one pass. v1.0.0. |
| [`security-prior-art-survey`](docs/skills/security-prior-art-survey.md) | Run the search wave of a systematic SECURITY prior-art survey — what will realistically be attacked in a product that does not exist yet, and what controls the evidence prescribes. Two procedures over schema-validated contracts: derive a threat-vocabulary map (6 group types, provenance + relation-kind expansions, negative terms, per-angle applicability verdicts, corpus release stamps, absent-input assumptions), and execute one of 8 angles (control standards / mobile standards / CWE→CAPEC→ATT&CK traversal / incident corpora / disclosure mining / ecosystem advisories / vendor CSAF-VEX feeds / supply-chain attack patterns) into status-typed coverage cells + class-appropriate candidate records. The did-not-look versus found-nothing distinction is structural: not-run and vacated outcomes forbid coverage cells outright, and only a `reached` cell may carry counts. NVD is corroboration-only (2026 enrichment narrowing); an embargoed bulletin is incomplete, not absent. Adds the EXTRACT wave: deep-read ONE source item into a nine-section analysis above a machine block whose evidence tier carries its receipts (catalog membership or a matching incident is tier 1; a proof-of-concept is tier 2 and severity never promotes between tiers), severity recorded per system and version rather than collapsed, the control the source itself prescribes (`stated: false` where it prescribes none — never invented), and `aliases` kept distinct from `related`. A cheap relevance skim bails only on a confident "applies to none of the scope" — uncertainty keeps the item. Adds the SYNTHESIS wave: aggregate extractions into a threat register keyed by externally-named threats (attack pattern first, weakness class where none fits — never coined), duplicates collapsed on aliases and never on related, each row's tier capped at its strongest evidence, and a coverage receipt that reports every non-firing angle so an absence is provable. A deterministic validator gates all four artifacts, including that each extract record's FILENAME encodes its identity — an item_id may be a stable URL, and a URL written verbatim as a filename lands the record where nothing looks for it while it stays perfectly valid. An item judged relevant but unreadable is a coverage GAP carrying its cause, never dropped like an irrelevant bail: it reaches the register's receipt, which is the only place it can appear. The receipt is reconciled against the search outputs, so an angle that ran and reached nothing cannot be summarised as one that never started. Each coverage cell balances exactly — `returned = kept + dropped + deduped` — so every result a source returned was carried forward, cut by the cap, or collapsed into an item already counted, and a relevance cut has nowhere to hide. v1.5.2. |
| [`reviewing-security-prior-art-survey`](docs/skills/reviewing-security-prior-art-survey.md) | Judge a produced security prior-art artifact — threat-vocabulary map or per-angle search output — against the thirty-eight-condition bar single-sourced with `security-prior-art-survey` (an acceptance gate, not authoring): the mechanical conditions discharged by one run of the co-installed producer's validator, judgment conditions walked with explicit gap-vs-not calibration, and a probe-asymmetry guard so a reviewer's own successful re-probe never becomes a finding against an honest `unreachable`. Stops the review outright when a search output arrives without its vocabulary map. Adds extract-record conditions 23-30 (bail integrity, deep-read fidelity, tier groundedness, severity as published, control provenance, alias-vs-related, honest limits, scoped surfaces), appended above the existing set so earlier condition references stay valid, plus register conditions 31-37 (threat naming, duplicate collapse, tier not promoted, control attribution, receipt completeness, dependency surface stated both ways, no manufactured authority). Emits `VERDICT: approve\|revise` with condition-named findings; review-only, no false-revise — a thin-but-honest result in a thin domain meets the bar. Adds C38 for an extract record skipped as unavailable or withdrawn, graded the way the search wave's typed failures already are — its absence is what left a reviewer improvising a bar for a shape the conditions never covered, and makes angle-verdict corroboration bidirectional — a wrong POSITIVE verdict spends a whole angle on a product the scope already ruled out. v1.4.0. |
| [`visual-prior-art-survey`](docs/skills/visual-prior-art-survey.md) | Run the search wave of a systematic VISUAL & INTERACTION prior-art survey — what the industry's documented conventions already prescribe for a product's domain, and what an accessibility standard actually requires — before any wireframe, design system or hi-fi screen exists. Two procedures over schema-validated contracts: derive a UI-pattern vocabulary map (five axes — component, pattern, screen-archetype, platform-context, design-system — with typed expansions, honest provenance, mandatory negative terms on the collision-prone design-system axis, per-angle applicability verdicts and absent-input assumptions), and execute one of 7 angles (design-system documentation traversal / ARIA-APG interaction-pattern specification / platform HIG retrieval / deceptive-pattern and enforcement corpus / accessibility-criterion deep retrieval / domain-convention mining / open-source UI documentation) into status-typed coverage cells. **It mines documentation, never screenshots** — the galleries are excluded both on their terms and, decisively, because a screenshot is not extractable into the markdown artifact the downstream wireframing skill consumes; that second ground would still hold if every gallery opened tomorrow. The did-not-look versus found-nothing distinction is structural: not-run and vacated forbid coverage cells outright, only a `reached` cell may carry counts, `kept` must reconcile against the rows that name it, and coverage completeness is checked in BOTH directions. Every candidate records who says it (`authority`) separately from whether it binds (`prescriptivity`) plus its `corpus_version`, because a design system's opinion stated in imperative prose is not normative and a collapsed credibility field would hide that downstream; a claimed `token_format` must be DTCG and versioned. Caps are sized per corpus rather than uniform — the accessibility angle's 90 exists because a cap below an enumerable set truncates a corpus that could have been covered completely. An always-on angle can be neither switched off nor starved: wave 0's active-source list intersects every later angle's applicable set, so a source left `skipped` is a source no angle can query. Deterministic validator, 44 rules, 109 tests; three planted fixtures pass it and are each wrong. v1.0.1. |
| [`reviewing-visual-prior-art-survey`](docs/skills/reviewing-visual-prior-art-survey.md) | Judge a produced visual prior-art artifact — UI-pattern vocabulary map or per-angle search output — against the twenty-seven-condition bar single-sourced with `visual-prior-art-survey` (an acceptance gate, not authoring). Mechanical conditions are discharged by one run of the co-installed producer's validator and deliberately not restated; the judgment conditions are the point — whether a zero is a zero or a failure wearing one, whether the cited corpus actually contains the convention claimed (a URL that resolves and a plausible release stamp are checkable by shape; whether the pattern page really specifies the keyboard contract asserted is not), whether `authority` and `prescriptivity` are recorded and not confused, whether `provenance: extracted` is honest given the map is built before the search, and whether `corpus_version` is meaningful and distinct from `as_of`. C26 guards the domain-neutrality limit: a record claiming a screen composition its cited corpus does not address is how a downstream wireframing skill inherits an invented requirement. Proportionality is a numbered condition, not a disposition: a thin-but-honest artifact is correct output for a narrow UI, and revising for thinness invites padding. Evidence is the artifact, its schemas and the source registry — never the reviewer's own taste in interfaces. Emits `VERDICT: approve\|revise` with condition-named findings, all in one pass. v1.0.0. |
| [`user-research-prior-art-survey`](docs/skills/user-research-prior-art-survey.md) | Run the search wave of a systematic PUBLISHED-USER-RESEARCH prior-art survey — what has already been studied about the people who will use a product, what those studies found, and how much of it is retrievable at all. Two procedures over schema-validated contracts: derive a research vocabulary map (four axes — user-population, task, method, component — with SKOS-typed expansions, honest provenance, mandatory negative terms on the collision-prone METHOD axis, per-angle applicability verdicts and a six-member per-source access_status), and execute one of 7 angles (open scholarly-index / practitioner-research corpus / regulated-domain / assistive-technology / platform usage-research / human-AI interaction / developer-experience) into status-typed coverage cells. The `method` axis is what makes this survey work and has no analogue in the sibling types: bibliographic corpora are indexed BY STUDY DESIGN, so 'diary study', 'think aloud' and 'task completion rate' reach work no topic term will. **Admission has two conjuncts** — full text retrievable without bypassing a paywall AND a stated method — because retrievable argument reports no study, while a method behind a wall yields a record built from an abstract that reads exactly like one grounded in the method section. Following a resolver to the full text is part of the mechanism, since the backbone index returns metadata only. Source access is verified per source and split by host: two corpora have a website and an API that answer differently and **the permitted half is not the same for both** — one refuses on the website, the other refuses on the API — so a cell names the host, never the corpus. Three sources declare crawl delays, and for those cells the selection IS the method: the record must state what was identified and deliberately NOT fetched. Counts are forbidden on a cell that retrieved nothing, so a throttle cannot masquerade as a zero. Deterministic validator, 48 rules, 104 tests, every rule mutation-checked; three planted fixtures pass it and are each wrong. v1.1.0. |
| [`reviewing-user-research-prior-art-survey`](docs/skills/reviewing-user-research-prior-art-survey.md) | Judge a produced published-user-research prior-art artifact — vocabulary map or per-angle search output — against the twenty-seven-condition bar single-sourced with `user-research-prior-art-survey` (an acceptance gate, not authoring). Mechanical conditions are discharged by one run of the co-installed producer's validator; the judgment conditions are the point — whether the method axis names study designs or topic terms in disguise, whether an admitted source's full text really exists AND really states a method (the two conjuncts fail in opposite directions, and checking one passes half the artifact), whether a crawl-delayed cell records what it chose NOT to fetch, and whether a zero is a zero, a failure, or a throttle. **C13 is explicitly a COHERENCE check**: an earlier version asked the reviewer to judge whether a status was TRUE, and a blind reviewer correctly approved a laundered-failure fixture, because a closed evidence set cannot see a coherent launder. It now enumerates the five internal contradictions that discharge it and admits one asymmetric channel — a re-probe is citable when it FAILS, never when it succeeds — and records the genuinely unreachable case as a stated limit rather than pretending to cover it. Proportionality is a numbered condition: a thin-but-honest artifact is correct for a thinly-researched domain. Emits `VERDICT: approve\|revise` with condition-named findings, all in one pass. v1.0.0. |
...
**Honest limits, where the previous wording promised more than the angles deliver.** A candidate from an excluded fetch target is admissible only where an open mirror is independently reachable — the metadata index links to the publisher and the citation graph can point back at the excluded host, so an on-scope record dying there is a real outcome rather than a failure. A resolver that refuses outright is neither a paywall nor abstract-only and now has its own outcome. The always-on scholarly angle's fallback covers retrieval only and does NOT restore the citation-graph leg, which is the angle's only mechanism for finding work the vocabulary did not name; the permitted arXiv paths carry no search endpoint, so that channel is a recency walk and a zero from it is not evidence a topic is unstudied.

**Schema:** `sanitization` gains `not-fetched`, because a posture established without retrieving content has no sanitization verdict and `unavailable` asserts the different thing that it was attempted and degraded. The search output's `meta` gains an optional `note`, so run-level context stops being pushed into `notes`, which is scoped to cross-angle leads.

**Reviewer conditions that could not be executed as written are now honest about their edges.** The `kept` condition no longer asks for a plausibility judgment about a corpus the reviewer may neither re-probe nor recall. The stated-method condition splits the half a reviewer can check (does the field read as a study design, or as a topic?) from the half it cannot (does the source itself name it?), which belongs to the wave that does the full read. A map/search contradiction now states which artifact's verdict carries it.

v2.52.0 — **a declared contract that nothing read, and a rule that could never fire** (`market-competitive` 1.1.0, `security` 1.5.2). Both surfaced while fixing the previous pass, and both are the same defect class one level apart: something that looks like a guarantee and is not one.

**`market-competitive` declared six `trigger_anchor` entries its validator never read.** The field guards the failure where a conditional angle rests on an optional capability field, so the predicate is false for every map that omitted it and the angle looks configured while never running — and the field asserting that this could not happen was itself unchecked. Worse, the anchors used the superseded SCALAR shape: `b6` rests on **two** required legs, and a scalar can name only one, so the assertion had stopped matching the predicate it claimed to check. All six predicates were audited per angle, the anchors are now lists, the three angles with optional disjuncts record them as `widening_legs` (an optional leg beside a required one only ADDS firings — it fails open, and is legitimate), and the validator now enforces list-ness, non-emptiness and required-rootedness. A registry fault exits **2** on both subcommands, matching the sibling packages.

**`security`'s `not-run-no-coverage` was dead code.** The schema's own `if/then/not` already forbids `coverage` and `candidates` on a `not_run` or `vacated` outcome, and the validator returns early on any schema failure — so the rule could not fire for any document the enum admits, which was verified exhaustively rather than argued. It is **removed** rather than tested: a check the schema short-circuits is dead code that reads as a guarantee, and the constraint is now pinned by tests against the schema, where it actually lives.

`market-competitive`'s `kept-matches-rows` — the arithmetic that stops rows being dropped without a record — also shipped with no test naming it, and now has two. Mutation audit re-run across both packages.

v2.51.0 — **two defects fixed across four prior-art producers** (`market-competitive` 1.0.1, `security` 1.5.1, `visual` 1.0.1, `user-research` 1.0.1). Both were found by a cold-agent dry-run of the user-research pair and are inherited, identical, by its siblings — which is why the fix lands in all four rather than only where it was noticed.

**`relation-variety` was checked per group and should have been checked per map.** The rule failed unless SOME group carried more than one relation kind, which is wrong in both directions: it false-FAILED a map whose groups were each uniform but collectively varied — three relation kinds across the map, no single group mixed — and it passed a map of pure spelling lists as long as one group happened to be mixed. A group may legitimately be uniform: every expansion of one term can be `narrower`, and a system's aliases are all `alt-label`. The check now asks whether the MAP carries more than one relation kind anywhere. It deliberately still does not catch a map that is mostly spelling lists with one varied group — judging whether expansions genuinely expand is semantic and belongs to the reviewing twin, not to a shape gate, and a fuzzy threshold inside a deterministic validator produces false failures.

**A registry fault reported itself as an artifact failure** (`visual`, `user-research`). The source registry ships inside the package, so a defect in it is a package fault — but `anchor_failures` ran inside the keyword-map validation and surfaced at exit 1, the code reserved for "a rule failed", telling a caller to go and edit a map that was perfectly fine. It was also asymmetric: the search subcommand never checked the registry at all. The check now runs before either subcommand touches its input and exits 2 with the rest of the could-not-be-used class.

Each fix ships with a regression test that fails against the old behaviour, and every rule in all four validators was re-checked by mutation audit.

v2.50.0 — **`user-research-prior-art-survey` + `reviewing-user-research-prior-art-survey` v1.0.0** — a new prior-art pair, this one for the PUBLISHED USER RESEARCH survey: what has already been studied about the people who will use a product, and how much of it is retrievable at all. Two authoritative JSON Schemas, a verified source registry doubling as the validator's cap and applicability input, seven per-angle briefs, an absent-input policy, two artifact guides, and a deterministic validator (48 rules, 104 tests). v1 covers the SEARCH wave. Its `method` axis has no analogue in the sibling types and is what makes the survey work: bibliographic corpora are indexed **by study design**, so "diary study" and "task completion rate" reach work no topic term will, and a method group restating the topic has lost the axis. **Admission has two conjuncts** — retrievable full text AND a stated method — which fail in opposite directions: retrievable practitioner argument reports no study, while a method behind a paywall yields a record built from an abstract that reads exactly like one grounded in the method section. Source access is verified per source and **split by host, with the permitted half differing per source**: one corpus refuses on its website and answers on its API, the other refuses on its API while its listing paths are expressly allowed — so a cell names the host it reached, never the corpus. arXiv's own Terms of Use and robots file contradict each other outright, and the registry records the conflict rather than adjudicating it, reaching the allowed paths instead. Three rules generalise past this type. **Counts are forbidden on a cell that retrieved nothing** — a `rate-limited` cell carrying `returned: 0` is one field-rename from a laundered failure. **The angle precondition is compared against the registry** the validator already loads, rather than left to a reviewer's eye. And **a review condition must not promise more than its evidence set can deliver**: the central zero-versus-failure condition originally asked whether a recorded status was TRUE, a blind reviewer correctly APPROVED a laundered-failure fixture under it, and it is now a coherence check with its discharging contradictions enumerated, one asymmetric re-probe channel (citable when it fails, never when it succeeds), and the unreachable case recorded as a stated limit. The gate was verified by a mutation audit — each rule's emission neutered in turn, 49 of 49 caught by a test — and by two cold-agent dry-runs on unrelated scopes, the second producing both artifacts clean on the first attempt. A blind reviewer revised all three planted fixtures under the intended conditions and additionally found a real defect in the producer's own baseline fixture, since fixed.

v2.49.0 — **`visual-prior-art-survey` + `reviewing-visual-prior-art-survey` v1.0.0** — a new prior-art pair, this one for the VISUAL & INTERACTION survey: what the industry's documented conventions already prescribe for a product's domain, and what an accessibility standard actually requires, before any wireframe exists. The producer ships two authoritative JSON Schemas, a machine-readable source registry doubling as the validator's per-angle cap and applicability input, seven per-angle briefs, an absent-input policy, two artifact guides, and a deterministic validator (`keyword-map` + `search`, 44 rules, 109 tests). v1 covers the SEARCH wave. The defining decision is the **modality lock**: this survey mines documentation and never screenshots, on two independently sufficient grounds — the galleries (Mobbin, Pttrns, Dribbble, Page Flows, Lapa Ninja) are subscription products whose terms exist to prevent automated extraction, and, decisively, **a screenshot is not extractable into the markdown artifact the downstream wireframing skill consumes**. The second ground is the durable one: it would still hold if every gallery opened its doors tomorrow. All five are recorded in the registry's `excluded` block with a verified date, and the validator rejects a coverage cell or a fallback naming any of them. Three rules here generalise past this type. **Caps are sized per corpus rather than uniform** — the accessibility angle's 90 exists because WCAG's success-criteria set is enumerable and a cap below an angle's enumerable set truncates a corpus it could have covered completely; a uniform cap would have silently cut it. **An always-on angle can be starved as well as switched off** — wave 0's `active` source list intersects every later angle's applicable set, so a source left `skipped` is a source no angle can query, and an always-on angle with no active source is forced to `vacated`, which is the survey silently doing nothing. And **a conditional trigger's REQUIRED-rooted legs are recorded separately from its optional ones** (`trigger_anchor` vs `widening_legs`): an optional disjunct only ever adds firings and is legitimate, but a predicate rooted solely on one fails closed and invisibly. The pair also keeps **authority** (who says it) apart from **prescriptivity** (whether it binds) rather than collapsing both into one credibility field, because a design system's opinion stated in imperative prose is not normative and downstream must be able to tell. Verified by a blind reviewer run against three planted fixtures that pass the validator and are each wrong: a degraded cell rewritten as a searched zero, a relevance line asserting a keyboard contract the cited pattern page does not carry, and a W3C normative pattern demoted to one organisation's descriptive opinion. All three were caught under the expected conditions (C12, C16, C19), and an earlier run surfaced two genuine defects in the producer's own fixtures. A separate cold-agent dry-run produced both artifacts first try; it also caught that the skill's own fixture matched the scope it was handed, inviting transcription rather than a real search, and a fabricated claim in the SKILL.md body — that an accordion is a composition of a disclosure, which the APG does not say and which loses the heading requirement the two contracts do not share.

v2.48.0 — **`market-competitive-prior-art-survey` + `reviewing-market-competitive-prior-art-survey` v1.0.0** — a new prior-art pair, this one for the MARKET survey. The producer ships two authoritative JSON Schemas, a machine-readable source registry doubling as the validator's per-angle cap and applicability input, eight per-angle briefs, an absent-input policy, two artifact guides, and a deterministic validator (`keyword-map` + `search`, 42 rules, 94 tests). v1 covers the SEARCH wave. Its organising idea is that **a market with no competitor and a search that never ran produce identical-looking output**, enforced structurally: `not_run` and `vacated` forbid coverage cells, only a `reached` cell carries counts, `kept` reconciles against the rows naming it, and coverage completeness is checked in both directions — a missing cell is an unexplained gap and a surplus one means the angle worked a sibling's channels. Source access is verified per source by direct fetch and encoded rather than remembered: G2 is excluded on its **Terms of Use** rather than robots (its robots file permits AI crawlers; the terms forbid extraction regardless), and a second-hand digest of these same sources had misreported two of them, which is why the registry records who verified what and when. Two rules generalise beyond this type: every conditional trigger must rest on a field the capability schema marks REQUIRED — an optional anchor fails closed and invisibly, which had shipped live in one angle — and the per-angle cap lives in the registry sized to the corpus it walks, since a cap the validator never reads is decorative. Verified by a blind reviewer run against three planted fixtures that pass the validator and are each wrong: a rate-limited cell rewritten as a searched zero, an admission whose first-party quote states no capability, and a map whose job axis paraphrases its category axis. All three were caught under the expected conditions, and the same run surfaced four genuine defects in the producer's own baseline fixtures. A separate cold-agent dry-run produced both artifacts first try and mutation-tested the gate before trusting its own pass, which is how three contradictions between shipped documents were found.

v2.41.0 — **`security-prior-art-survey` + `reviewing-security-prior-art-survey` v1.0.0** — a new prior-art pair, this one for the SECURITY survey: what will realistically be attacked in a product that does not exist yet, and what controls the evidence prescribes. The producer ships two authoritative JSON Schemas, a machine-readable 25-source registry doubling as the validator's per-angle applicability input, eight per-source angle briefs, an absent-input policy, and a deterministic validator (`keyword-map` + `search` subcommands, 45 tests). v1 covers the SEARCH wave only; extraction and synthesis land later. The pair's organising idea is that **"we did not look" and "we looked and found nothing" are different facts**, and it is enforced structurally rather than by exhortation: the search-output schema is a discriminated union whose `not_run` and `vacated` outcomes forbid coverage cells outright, only a `reached` cell may carry counts, and the other five cell statuses each owe a cause — `unreachable` additionally owing the registry fallbacks it tried, so it cannot become the cheap exit from a merely slow source. Two source policies are encoded rather than remembered: the national vulnerability registry is corroboration-only after its 2026 enrichment narrowing, and an embargoed cloud bulletin is incomplete rather than absent. Two angle preconditions deliberately run on silence (mobile posture, supply-chain exposure), because in security under-coverage is the invisible failure. The reviewer's twenty-two conditions are the single anchor of the bar — the producer points at them and restates nothing normative, after the code pair recorded its own bar drifting across three unanchored mirror sites. Forged through five cold fresh-reviewer cycles: fifteen Criticals across the first four, zero in the fifth, and nearly every one of the fifteen was the same defect reappearing one level down — angle, then coverage cell, then the status added to fix the cell, then the map-side denominator, then the empty applicable set. Verified by a planted-defect gate: a map recording a group type absent on a reason the scope contradicts, and a search output retyping an unreachable cell as a reached zero with its retrieval summary cleaned to match — both pass the validator, both correctly REVISED under the expected conditions, and the same run surfaced two genuine defects in the producer's own reference fixtures (since fixed).
